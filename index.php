...
<body class="bg-slate-900 min-h-screen flex items-center justify-center p-4 font-sans text-white">
    ...
    <div class="w-full max-w-md bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700">
        ...
        <?php if($erro): ?>
            <div class="mb-4 p-3 bg-red-900/30 border border-red-500/50 rounded-lg text-red-300 text-sm text-center font-medium">
                 <?= $erro ?>
            </div>
        <?php endif; ?>
        ...
        <form method="POST" id="authForm" class="space-y-4">
            ...
            <div id="cadastroFields" class="hidden space-y-4 fade-in">
                <div>
                    ...
                    <input type="text" name="full_name" class="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white focus:border-amber-400 outline-none transition placeholder-slate-600" placeholder="Como quer ser chamado?">
                </div>
                <div>
                    <label class="block text-xs text-amber-400 mb-1 ml-1 font-bold uppercase">Meta Financeira (R$)</label>
                    <input type="number" name="meta" class="w-full bg-slate-900 border border-amber-500/50 rounded-lg p-3 text-white focus:border-amber-400 outline-none transition placeholder-slate-600" placeholder="Ex: 5000">
                </div>
            </div>

            <div>
                <label class="block text-xs text-slate-400 mb-1 ml-1 font-semibold uppercase">E-mail</label>
                <input type="email" name="email" required class="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white focus:border-amber-400 outline-none transition placeholder-slate-600" placeholder="seu@email.com">
            </div>

            <div>
                <label class="block text-xs text-slate-400 mb-1 ml-1 font-semibold uppercase">Senha</label>
                <input type="password" name="password" required class="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white focus:border-amber-400 outline-none transition placeholder-slate-600" placeholder="******">
            </div>
